...
function Command({
  ...
  return (
    <CommandPrimitive
      data-slot="command"
      className={cn(
        "bg-white dark:bg-[#0a0a0a] text-zinc-900 dark:text-zinc-50 flex h-full w-full flex-col overflow-hidden rounded-md",
        className
      )}
      {...props}
    />
...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent
        className={cn(
          "overflow-hidden p-0 border-neutral-100 dark:border-neutral-900",
          className
        )}
...
function CommandItem({
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "data-[selected=true]:bg-zinc-100 dark:data-[selected=true]:bg-zinc-800 data-[selected=true]:text-zinc-900 dark:data-[selected=true]:text-zinc-50 [&_svg:not([class*='text-'])]:text-zinc-500 dark:[&_svg:not([class*='text-'])]:text-zinc-400 relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />
